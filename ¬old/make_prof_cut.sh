...
$cncfc_dir/dxf2knots.py -i test_1.dxf -l prof_02_cutoff
$cncfc_dir/knots2gcode.py -i prof_02_cutoff1.knt -o prof_02_cutoff -d 418 -cm -sh

for FILE in prof_xyuv_*.knt
do
  name="${FILE%%.knt}"
  prof_numb="${name##'prof_xyuv_'}"
  prof_r="prof_r_$prof_numb.knt"

  # echo found matching:
    if [ -f "$prof_r" ]; then
      echo found matching: "$prof_r"

       $cncfc_dir/knots2gcode.py -i $FILE -ir $prof_r -o "prof_$prof_numb" -d 418 -cm -sh

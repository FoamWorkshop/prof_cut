...
for FILE in prof_03_xyuv_*.knt
do
  name="${FILE%%.knt}"
  prof_numb="${name##'prof_03_xyuv_'}"
  prof_r="prof_03_r_$prof_numb.knt"
  # echo found matching:
    if [ -f "$prof_r" ]; then
      echo found matching: "$prof_r"

      $cncfc_dir/knots2gcode.py -i $FILE -ir $prof_r -o "prof_03_$prof_numb" -d 418 -cm -sh
